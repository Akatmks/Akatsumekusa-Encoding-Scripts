...
* Progression Bosot by default uses ffms2 as video provider, but you can easily switch to BestSource, lsmas or other video providers in the file itself.  
...
* Additionally, to ensure a better quality, all presets by default use x264 + WWXD + Diff based scene detection methods. You would need to download x264, vanilla or mods, from [GitHub Release](https://github.com/jpsdr/x264/releases), and place them where av1an would recognise, You would also need to download WWXD from vsrepo (`wwxd`) or AUR ([WWXD](https://aur.archlinux.org/packages/vapoursynth-plugin-wwxd-git)).  
